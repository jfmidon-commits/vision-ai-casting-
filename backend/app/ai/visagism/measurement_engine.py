...
class VisagismMeasurementEngine:
    ...
    LANDMARKS = {
        ...
        'mouth_left': 61,
        'mouth_right': 291,
        # 0/17 medem o contorno vertical externo dos labios. 13/14 medem
        # a abertura interna da boca e nao devem ser usados como altura labial.
        'mouth_top': 0,
        'mouth_bottom': 17,
        'mouth_inner_top': 13,
    ...
    def __init__(self):
        ...
        self._image_width = 1.0
        self._image_height = 1.0
        self._image_diagonal = math.sqrt(2.0)
    ...
    async def measure_photo(
        ...
        # FaceMesh retorna x/y normalizados por largura/altura separadamente.
        # Distancias euclidianas diretamente nesses valores distorcem imagens
        # nao quadradas. Convertemos para pixel-space e normalizamos pela
        # diagonal da imagem para preservar comparabilidade entre resolucoes.
        self._image_width = float(max(image.width, 1))
        self._image_height = float(max(image.height, 1))
        self._image_diagonal = math.hypot(self._image_width, self._image_height)
    ...
    def _extract_measurements(self, photo_id: UUID, landmarks: List[Dict]) -> List[FacialMeasurement]:
        ...
        defs = [
            ...
            ("mouth_height", [m['mouth_top'], m['mouth_bottom']], "altura externa dos labios"),
        ...
        if all(k in m for k in ['forehead_height', 'nose_height', 'chin_height']):
            ...
            if total > 0:
                for key, label, ideal in [
                    ...
                    measurement_key = key.replace('_third', '_height').replace('upper', 'forehead').replace('middle', 'nose').replace('lower', 'chin')
                    val = m[measurement_key].value / total
                    proportions.append(self._create_proportion(photo_id, key, val, [m[measurement_key].evidence_id], ideal, label))
    ...
    def _classify_face_shape(self, proportions: List[FacialProportion]) -> FaceShape:
        ...
        h_w = hw.value
        j_f = jaw.value if jaw else 0.80
        c_j = chin.value if chin else 0.65
        f_f = forehead.value if forehead else 0.82

        if h_w >= self.RATIO_OBLONG:
            return FaceShape.OBLONG
        if h_w <= self.RATIO_ROUND_MAX and j_f >= self.JAW_SQUARE - 0.05:
            return FaceShape.ROUND
        if f_f >= 0.85 and c_j <= 0.60:
            return FaceShape.HEART
        if j_f <= self.JAW_DIAMOND and f_f <= 0.78:
            return FaceShape.DIAMOND
        if j_f >= self.JAW_SQUARE and abs(h_w - 1.0) < 0.15:
            return FaceShape.SQUARE
        if j_f > f_f + 0.05:
            return FaceShape.TRIANGULAR
        if self.RATIO_OVAL_MIN <= h_w <= self.RATIO_OVAL_MAX and 0.70 <= j_f <= 0.85:
    ...
    def _assess_regions(self, photo_id: UUID, landmarks: List[Dict], measurements: List[FacialMeasurement]) -> Dict[str, Any]:
        ...
        arch_raw = self._vertical_distance(landmarks[105], landmarks[52]) if len(landmarks) > 105 else 0
    ...
    def _distance(self, p1: Dict, p2: Dict) -> float:
        """Distancia 2D corrigida pelo aspect ratio e normalizada pela diagonal."""
        dx = (p1['x'] - p2['x']) * self._image_width
        dy = (p1['y'] - p2['y']) * self._image_height
        return math.hypot(dx, dy) / self._image_diagonal

    def _vertical_distance(self, p1: Dict, p2: Dict) -> float:
        return abs(p1['y'] - p2['y']) * self._image_height / self._image_diagonal
    ...
    def _safe_ratio(self, measurements: List[FacialMeasurement], num_name: Optional[str], den_name: Optional[str]) -> Optional[float]:
        m = {meas.name: meas for meas in measurements}
        if den_name is None:
            return m[num_name].value if num_name and num_name in m else None
    ...
    def _assess_eye_shape(self, landmarks: List[Dict]) -> str:
        if len(landmarks) > 374:
            a = abs(landmarks[159]['y'] - landmarks[145]['y']) * self._image_height
            b = abs(landmarks[158]['y'] - landmarks[153]['y']) * self._image_height
            c = abs(landmarks[33]['x'] - landmarks[133]['x']) * self._image_width
