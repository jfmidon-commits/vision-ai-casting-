...
class MediaPipeService:
    ...
    async def analyze_face_mesh(self, image) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "facial_proportions": self._calculate_facial_proportions(
                    landmarks, image_width, image_height
                ),
                "face_shape": self._classify_face_shape(
                    landmarks, image_width, image_height
                ),
                "eye_aspect_ratio": self._calculate_ear(
                    landmarks, image_width, image_height
                ),
                "mouth_aspect_ratio": self._calculate_mar(
                    landmarks, image_width, image_height
                ),
            }
        except Exception as exc:
    ...
    def _calculate_symmetry(self, landmarks: List[Dict]) -> float:
        # Use only true left/right homologous pairs. The previous list included
        # vertical center pairs (0,17) and (10,152), which artificially lowered
        # the score because they are not symmetry counterparts.
        symmetry_pairs = [
    ...
    def _pixel_distance(
        ...
        """Distance in pixel-space so x/y are comparable on non-square images."""
        dx = (p1["x"] - p2["x"]) * image_width
        dy = (p1["y"] - p2["y"]) * image_height
        return float(np.hypot(dx, dy))

    def _calculate_facial_proportions(
        self,
        landmarks: List[Dict],
        image_width: int,
        image_height: int,
        ...
        diagonal = float(np.hypot(image_width, image_height)) or 1.0
        face_width = self._pixel_distance(
            landmarks[234], landmarks[454], image_width, image_height
        ) / diagonal
        face_height = self._pixel_distance(
            landmarks[10], landmarks[152], image_width, image_height
        ) / diagonal
    ...
    def _classify_face_shape(
        ...
        jaw_width = self._pixel_distance(
            landmarks[58], landmarks[288], image_width, image_height
        )
        brow_width = self._pixel_distance(
            landmarks[105], landmarks[334], image_width, image_height
        )
        chin_width = self._pixel_distance(
            landmarks[148], landmarks[377], image_width, image_height
        )
        ...
        forehead_to_face = brow_width / face_width
        chin_to_jaw = chin_width / jaw_width

        # Keep the same deterministic rule order/thresholds as
        # VisagismMeasurementEngine so both layers report one canonical shape.
        if height_to_width >= 1.50:
            return "oblong"
        if height_to_width <= 1.20 and jaw_to_face >= 0.83:
            return "round"
        if forehead_to_face >= 0.85 and chin_to_jaw <= 0.60:
            return "heart"
        if jaw_to_face <= 0.70 and forehead_to_face <= 0.78:
            return "diamond"
        if jaw_to_face >= 0.88 and abs(height_to_width - 1.0) < 0.15:
            return "square"
        if jaw_to_face > forehead_to_face + 0.05:
            return "triangular"
        if 1.20 <= height_to_width <= 1.40 and 0.70 <= jaw_to_face <= 0.85:
            return "oval"
        return "mixed"
    ...
    def _calculate_ear(
        ...
        a = self._pixel_distance(left_eye[1], left_eye[5], image_width, image_height)
        b = self._pixel_distance(left_eye[2], left_eye[4], image_width, image_height)
        c = self._pixel_distance(left_eye[0], left_eye[3], image_width, image_height)
    ...
    def _calculate_mar(
        ...
        height = abs(mouth[2]["y"] - mouth[3]["y"]) * image_height
        width = abs(mouth[0]["x"] - mouth[1]["x"]) * image_width
